...
function renderInsights(insights) {
            ...
            const interval = setInterval(async () => {
                ...
                if (progress === 100) {
                    ...
                    try {
                        ...
                        const res = await fetch('http://localhost:5000/api/papers/generate-rescue', {
                            method: 'POST',
                            headers: { 
    ...
    if (rankCard && rankModal) {
        rankCard.addEventListener('click', async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const res = await fetch('http://localhost:5000/api/analytics/dashboard/summary', {
                    headers: { 'x-auth-token': token }
                });
    ...
    if (cogCard && cogModal) {
        cogCard.addEventListener('click', async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const res = await fetch('http://localhost:5000/api/analytics/dashboard/cognitive', {
                    headers: { 'x-auth-token': token }
                });
